...
function GeneralCourse() {
  ...
  return (
    <div className={styles['general-course']}>
      ...
      <button type="button" className={styles['question-icon']} onClick={openTooltip}>
        <QuestionMarkIcon />
      </button>
      <div className={styles.list}>
        {requiredEducationArea.map((track) => (
          <div key={track.course_type} className={styles.course}>
            <button type="button" onClick={() => handleOpenModal(track.course_type)} className={styles.course__button}>
      ...
      {isTooltipOpen && (
        <div className={styles.tooltip}>
          <div className={styles['tooltip-content']}>
            교양 영역을 클릭해서
            <strong> 학기 교양 개설 목록</strong>을 확인할 수 있어요.
          </div>
          <button type="button" aria-label="close" className={styles['tooltip-close']} onClick={closeTooltip}>
            <CloseIcon />
          </button>
