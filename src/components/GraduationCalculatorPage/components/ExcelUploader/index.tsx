...
function ExcelUploader() {
  ...
  const { handleFileUpload, handleDrop, handleDragOver } = useExcelUpload();
  ...
  return (
    <div className={styles['excel-uploader']}>
      <button type="button" onClick={openTooltip} className={styles['excel-uploader__description-button']}>
        <QuestionMarkIcon />
      </button>
      <label className={styles['excel-uploader__button']} onDrop={handleDrop} onDragOver={handleDragOver}>
        <UploadIcon />
        <span>엑셀파일 추가하기</span>
        <input
          type="file"
