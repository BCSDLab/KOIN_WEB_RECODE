...
function IndexBus() {
  ...
  const logShuttleTicket = () =>
    logger.actionEventClick({ actionTitle: 'CAMPUS', title: 'shuttle_ticket', value: '버스' });
  const logMainToBus = () =>
    logger.actionEventClick({ actionTitle: 'CAMPUS', title: 'main_to_bus', value: '버스' });
  const logMainBusButton = (key: BusLinkKey) =>
    logger.actionEventClick({ actionTitle: 'CAMPUS', title: loggingTitle[key], value: '버스' });
  ...
  return (
    <section className={styles.template}>
      <div className={styles.template__title}>
        <Link to={ROUTES.BusRoute()} onClick={logMainToBus}>
          버스
        </Link>
        {isMobile && (
          <Link to={unibus.link} className={styles.unibus} onClick={logShuttleTicket}>
            <QRCode />
      ...
      {isMobile ? (
        <div className={styles.cards}>
          {BUS_LINKS.slice(0, 2).map(({ key, title, subtitle, link }) => (
            <Link to={link} key={key} className={styles.card} onClick={() => logMainBusButton(key)}>
