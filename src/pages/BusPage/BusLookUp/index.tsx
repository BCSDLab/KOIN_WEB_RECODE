      ...
      <div className={styles.cards}>
        {BUS_TYPES.map(({ key: type }, idx) => (
          <React.Fragment key={type}>
            <div
              className={cn({
                [styles['cards__top-card']]: true,
                [styles[`cards__card--${type}`]]: true,
              })}
            >
              <div className={styles.cards__content}>
                <div>
                  <div className={styles.cards__direction}>
                    <span className={styles.cards__badge}>출발</span>
                    {depart.options[0]}
                  </div>
                  <div className={styles.cards__direction}>
                    <span className={styles.cards__badge}>도착</span>
                    {arrival.options[0]}
                  </div>
                </div>
                {busData[idx]?.now_bus?.bus_number && (
                  <span
                    className={styles['cards__bus-number']}
                  >{`${busData[idx]?.now_bus?.bus_number}번 버스`}</span>
                )}
              </div>
              <div
                className={cn({
                  [styles['cards__content--description']]: true,
                  [styles.cards__content]: true,
                })}
              >
                <div>
                ...
                <div>
                  <img
                    className={styles['cards__bus-icon']}
            ...
            <div
              className={cn({
                [styles['cards__bottom-card']]: true,
                [styles[`cards__card--${type}`]]: true,
              })}
            >
              <div className={styles.cards__content}>
                ...
                {busData[idx]?.next_bus?.bus_number && (
                  <span
                    className={styles['cards__bus-number']}
                  >{`${busData[idx]?.next_bus?.bus_number}번 버스`}</span>
                )}
              </div>
              <div className={styles.cards__content}>
                <div>
