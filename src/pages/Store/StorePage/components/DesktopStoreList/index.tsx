      ...
      {storeListData?.map((store: StoreList) => (
        <Link
          ...
          onClick={() => logger.actionEventClick({
            actionTitle: 'BUSINESS', title: `${storeType === 'store' ? 'shop' : 'benefit_shop'}_click`, value: store.name, event_category: 'click', previous_page: `${koreanCategory || '전체보기'}`, current_page: `${store.name}`, duration_time: (new Date().getTime() - Number(sessionStorage.getItem('enter_category'))) / 1000,
          })}
        >
          {store.is_event
            && store.is_open
