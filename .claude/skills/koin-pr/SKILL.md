...
8. Create or reuse a branch named `{type}/#{issue-number}/{english-kebab-case}` (`{type}` = `feat`/`fix`/`refactor`/`test`/`docs`/`chore`; description in English). Base new work on the GitHub default branch, currently `develop` — hotfix branches target `main` instead and use `hotfix` as `{type}` (e.g. `hotfix/#1462/nearby-store-desktop-link`), regardless of the underlying work type.
